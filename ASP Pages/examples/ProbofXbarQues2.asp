...
<HTML>
<HEAD><TITLE>Probability of Sample Means</TITLE>


</HEAD>
<BODY>
<form action=ProbOfXbarSol.asp method=POST enctype="application/x-www-form-urlencoded">
<%
   Dim Question()
   ...
   'Creation of scenarios
    ...
    'Determing a random < range and its prob
        ...
        LessThanValue = round(mu + z * SE,2)
        LessThanProb = alnorm(z)
        Question(1) = "1. What is the probability of finding a sample mean" & _
                  " less than " & LessThanValue & " " & mea & "?"
      
      'Determing a random > range and its prob
        z = Round((Rnd() - 0.5) * 6, 2)
        GrThanValue = round(mu + z * SE,2)
        GrThanProb = 1 - alnorm(z)
        Question(2) = "2. What is the chance of finding a sample average with a value " &  _
                   " greater than " & GrThanValue & " " & mea & "?"
           
    'Determining a random range and then finding its prob
        z = round(RandBetween(0.05, 3),2)
        
        uppervalue = round(mu + z * SE,2)
        lowervalue = round(mu - z * SE,2)
        withinValue = round(mu-lowervalue,2)
        BetweenProb = alnorm(z) - alnorm(-1*z)
        Question(3) = "3. What is the probability of finding a sample average within " & withinValue & _
          " " & mea & " of the population mean?"
       'prhtml("z=" & z "<br>se = " & se & "<br>moe = " & z*se & "<br> U =" & uppervalue & "<br> L = " & lowervalue)
    ...
    'Determining a determining middle proportion and then finding the two xbar values 
        ...
        prhtml("<p> se = " & se & ", " & "moe = " & se*z1_for_pctile & "</p>")
        ...
        Question(4) = "4. What two values of the sample mean cut off the center " & CenterProb*100 & "% of the sampling distribution?"
     ...
     ". You plan to take a random sample of " & n & " " & obsr & " and calculate a sample mean. "
...
%>
 ...
 <h1 align="center"> Exercise on the Probability of a Normally Distributed Sample Average</h1>
