...
<HTML>
<HEAD><TITLE>Z CI on Mu</TITLE>


</HEAD>
<BODY>
<form action=ZCIforMuSol.asp method=POST enctype="application/x-www-form-urlencoded">
<%
   Dim Question()
   ...
   Randomize 
    ...
    'Determining a determining middle proportion and then finding the two xbar values 
        ...
        CL = Round(randbetween(.8,.99),2) 
        'Find area less than the upper range
        UpperPercentile = 1.-(1-Cl)/2   
        Z = round(NormSInv(upperPercentile),2)
        'xbar=abs(round(mu+round(normsinv(rnd()*.9+.05),2)*se,2))

        LowerB = round(xbar - z * SE,2)
        UpperB = round(xbar + z * SE,2)
        'prhtml("<p> z = " & z & " </p>")
     ...
     paragraph = "Estimate the mean "& x &" of all " & obsr & " with " & CL*100 & "% " & _
       "confidence. After collecting a random sample  of " & n & " " & _
        obsr &" you find a sample mean of " & xbar & " " & mea & ". Assume the " & _
       "distribution of " & x & " for all " & obsr & " is normal with " & _
       "a standard deviation of " & sig & " " & mea & "."
...
%>
 ...
 <h1 align="center"> Exercise on the Z Confidence Interval for the Population Mean</h1>
